...
public class MenuFontAsmHack {
	...
	private static int writeMenuSpriteFuncAddr,writeGETINSpriteFuncAddr;
	...
	private static void hackMenuFont(String splitDir) throws IOException{
		...
		main.seek(RamFaceEditor.OTHER_OFFSET);
		writeMenuSpriteFuncAddr = Conf.SCRIPT1_ADDR+RamFaceEditor.OTHER_OFFSET; //warning: asm/uvRouter.asm倒数第2行 j xxxx应当改成 writeMenuSpriteFuncAddr
		System.out.println(Integer.toHexString(writeMenuSpriteFuncAddr));
		...
		if(writeMenuSprite.length+writeGETINSprite.length>RamFaceEditor.OTHER_CAPACITY){
			throw new UnsupportedOperationException("writeMenuSprite.asm and writeGETINSprite.asm are too large");
		}
