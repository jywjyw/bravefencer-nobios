 ...
 * 脚本内容:找到狗后与枷台约翰的对话
 */
public class ScriptHandlerSC03_061_01 implements Importer,Exporter{
	String splitDir;
	Script script;
	...
	public List<String> import_(Encoding1 enc1, List<Sentence> sentences, List<String> parentChars)
			throws IOException {
		new ImporterScript3ActiveEvent(0, scriptAddrGetter, splitDir, script){	
			@Override
			public void modifyFontPointer(RandomAccessFile file, int newFontOffset) throws IOException {
				RandomAccessFile parent = new RandomAccessFile(splitDir+script.parent.file,"rw");
				int newMemAddr = scriptAddrGetter.getStartAddr()+newFontOffset;
				Util.assertTrue((newMemAddr&0xFFFF0000)==0x801F0000);
				int offsetAddr=newMemAddr&0xffff;
				if(offsetAddr>0x8000){	//mips指令addiu：，如果偏移地址>=0x8000，会变成负数
					parent.seek(0x5E6D0);
					parent.write(0x20);		//基准值改为0x8020
				}
				parent.seek(0x5E6D4);
				parent.write(new byte[]{(byte) (newMemAddr&0xff),(byte) (newMemAddr>>>8&0xff)});
				parent.close();
			}
