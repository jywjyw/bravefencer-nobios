 ...
 *
 */
public class ScriptHandlerSC03_060_01 implements Importer,Exporter{
	String splitDir;
	Script script;
	...
	public List<String> import_(Encoding1 enc1, List<Sentence> sentences, List<String> parentChars)
			throws IOException {
		new ImporterScript3ActiveEvent(0, scriptAddrGetter, splitDir, script){	//与其它第3脚本的扩容方式相似,但是有多个字库指针
			@Override
			public void modifyFontPointer(RandomAccessFile file, int newFontOffset) throws IOException {
				...
				if((newMemAddr&0xFFFF0000)!=0x801F0000) throw new RuntimeException();
				for(int i:new int[]{0x14,0x178,0x220,0x340,0x520}){
					if((newMemAddr&0xffff)>0x8000){	//mips指令addiu：，如果偏移地址>=0x8000，会变成负数
						file.seek(i-4);
						file.write(0x20);		//基准值改为0x8020
					}
					file.seek(i);
					file.write(new byte[]{(byte) (newMemAddr&0xff),(byte) (newMemAddr>>>8&0xff)});
				}
			}
